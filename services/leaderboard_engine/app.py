...
def get_leaderboard():
    try:
        ...
        match_response = http_client.get(
            f"{MATCH_ENGINE_URL}/internal/scores/top", timeout=2.0
        )
        ...
        for score_record in raw_scores:
            ...
            try:
                player_response = http_client.get(
                    f"{PLAYER_ENGINE_URL}/internal/players/{player_id}", timeout=1.5
                )
                if player_response.status_code == 200:
                    username = (
                        player_response.json()
                        .get("data", {})
                        .get("username", "Unknown Player")
                    )
                else:
                    username = "Unknown Player"
            except requests.RequestException as e:
                print(
                    f"DEBUG Error: Could not resolve player {player_id}. Details: {e}"
                )
                username = "Unknown Player"

            ranks.append(
                {"player_id": player_id, "username": username, "high_score": high_score}
            )
        ...
        if ranks:
            try:
                ...
                ai_response = http_client.post(
                    f"{AI_ENGINE_URL}/internal/generate/hype", json=payload, timeout=5.0
                )
                if ai_response.status_code == 200:
                    ai_hype_message = (
                        ai_response.json()
                        .get("data", {})
                        .get("hype_message", ai_hype_message)
                    )
                else:
                    print(
                        f"DEBUG Error: AI Engine returned status code {ai_response.status_code}"
                    )
            except requests.RequestException as e:
                # Diagnostics: Print why the AI connection is failing
                print(
                    f"DEBUG Error: Connection to AI Engine failed at {AI_ENGINE_URL}. Details: {e}"
                )
            ...
            jsonify(
                {
                    ...
                    "data": {"ranks": ranks, "ai_hype_message": ai_hype_message},
                }
            ),
            200,
        )
...
def health_check():
    return (
        jsonify(
            {"success": True, "service": "leaderboard_engine", "status": "healthy"}
        ),
        200,
    )
